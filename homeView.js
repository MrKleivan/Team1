...
function updateViewHome(count) {
    ...
    if(model.inputs.home.svipeList.length < 1) {
        ...
        <div id="homeContent"  style="width: 100%; height: 80%; margin: auto;align-items: center; display: flex; align-content:center; text-align: center;">
            <div style="width: 10%; height: 95%; align-content:center;" >
            <img style="height: 5vh" src="img/leftArrow.svg" alt="Left arrow"  onclick="privPic(counter)" />
            </div>
            <div id="homeProfile" style="width: 95%; height: 90%; border-radius: 40px; align-content: center; background-color: rgba(255, 255, 255, 0.5);">
                <div class="profileView" onclick="navigateToPage('otherProfile')" style=" width: 98%; height: 98%; margin: auto; border-radius: 40px; background-image: url(${catPicture[counter].pictureUrl});background-size: cover;background-position: center; background-repeat: no-repeat;">
            ...
            <div style="width: 10%; height: 10%; align-content:center;">
            <img style="height: 5vh" src="img/rightArrow.svg" alt="Right arrow"  onclick="nextPic(counter, catPicture)" />
            </div>
        </div>
        <div style="text-align: center;">
            ${createPictureCircle(catPicture, counter)}
        </div>
        <br>
...
function createPictureCircle(catPicture, counter) {
    let html = '';
    for(i = 0; i < catPicture.length; i++){
        html += /*HTML*/`<span>
            <svg width="30" height="30">
                <circle cx="15" cy="15" r="10" fill="${counter == i ? `yellow` : `grey`}" />
            </svg> 
        </span>`;
    }
    return html;
